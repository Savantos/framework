	...
	tinymce.create('tinymce.plugins.tf_food_menu_shortcode_plugin', {
		...
		_do_foodmenu_shortcode_image_replace : function(co) {
			...
			return co.replace(/(\[tf-menu-([^\]]*)\])/g, function(a,b) {
				...
				return '<input type="button" class="tfFoodMenuShortcode" data-shortcode="'+encodedShortode+'" style="background:#f2f2f2; background:-webkit-linear-gradient(#f2f2f2,#ccc); border:1px solid #ddd; padding:3%; border-radius: 5px; margin: 10px 0; display:block; height: 75px; color: black; text-align:left; font-weight: bold; ' + style + '" value="Food Menu '+ args.type + ': ' + args.id + '" />';
			});
