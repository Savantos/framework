(function() {
	...
	tinymce.create('tinymce.plugins.tf_fb_events_shortcode_plugin', {
		/**
		...
		init : function(ed, url) {
			...
			ed.addCommand('mceExecTFEventsInsertShortcode', function() {
				...
				t.activeShortcode = '';
				var argUrl = url + '/insert_fb_shortcode_dialog.php?' + jQuery.param( args );
			...
			ed.onMouseDown.add(function(ed, e) {
				...
				if ( e.target.nodeName == 'INPUT' && ed.dom.hasClass(e.target, 'tfFBEventsShortcode') ) {
					ed.plugins.wordpress._showButtons(e.target, 'tf_fb_events_btns');
					
					ed.selection.select(e.target);
					
				} else {
					tinymce.DOM.hide('tf_fb_events_btns');
				}
		...
		_do_events_shortcode_from_content : function(co) {
			...
			return co.replace(/<input class="tfFBEventsShortcode([^\/]*?)\/>/g, function(a,b){
		...
		_do_events_shortcode_image_replace : function(co) {
			...
			return co.replace(/(\[tf-fb-events-([^\]]*)\])/g, function(a,b) {
		...
		_createButtons : function() {
			...
			DOM.remove('tf_fb_events_btns');
			
			DOM.add(document.body, 'div', {
				id : 'tf_fb_events_btns',
				style : 'display:none;position:absolute;'
			});

			editButton = DOM.add('tf_fb_events_btns', 'img', {
				src : t.url+'/event_16.png',
				id : 'tf_fb_events_btn_edit',
				width : '20',
				height : '20',
				title : "Edit Events",
			...
			dellButton = DOM.add('tf_fb_events_btns', 'img', {
				src : t.url+'/img/delete.png',
				id : 'tf_fb_events_btn_del',
				width : '20',
				height : '20',
			...
			tinymce.dom.Event.add(dellButton, 'mousedown', function(e) {
				...
				if ( el.nodeName == 'INPUT' && ed.dom.hasClass(el, 'tfFBEventsShortcode') ) {
					ed.dom.remove(el);
					tinymce.DOM.hide('tf_fb_events_btns');
		...
		getInfo : function() {
			return {
			        longname : 'ThemeForce Facebook Events Shortcode',
			        author : 'ThemeForce',
	...
	// Register plugin
	tinymce.PluginManager.add('tf_fb_events_shortcode_plugin', tinymce.plugins.tf_fb_events_shortcode_plugin);
